...
import { Bookmark, Trash2, BookOpen, Library as LibIcon, Stethoscope, GraduationCap, Target } from "lucide-react";
...
export function Salvos() {
  ...
  const [filtro, setFiltro] = React.useState<"todos" | BookmarkType | "aplicacoes">("todos");

  const grupos = ORDEM.map((t) => ({ t, items: bookmarks.filter((b) => b.type === t) })).filter((g) => g.items.length > 0);
  const total = bookmarks.length + applications.length;

  const mostrarAplic = filtro === "todos" || filtro === "aplicacoes";
  const gruposFiltrados = filtro === "todos" ? grupos : filtro === "aplicacoes" ? [] : grupos.filter((g) => g.t === filtro);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <SectionHeader
        ...
        title="Salvos"
        subtitle="Conteúdos, disciplinas, casos, referências e justificativas que você guardou para consultar depois."
      />
